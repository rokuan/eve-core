 ...
 */
class StreamReceiver(private val receiverId: String, protected val handler: SocketLockHandler, protected val receiver: Receiver) extends Receiver with QuerySource with ObjectStreamSource {
    ...
    handler.resultProcess {
      command match {
        case c: GetMappingsCommand => {
          ...
          handler.writeObjectResponse(mappings)
        }
